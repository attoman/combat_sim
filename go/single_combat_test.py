# -*- coding: utf-8 -*-
"""
전체 항공기 모델 및 1 대 1 BFM 시뮬레이션 코드 (v4 - 최종 통합본)

이 파일은 이전에 논의된 모든 클래스 및 함수 정의를 통합하고 중복을 제거한 최종 버전입니다.
F-16 vs UAV 1 대 1 BFM 시뮬레이션을 실행하는 예제를 포함하며, 상태 머신 기반 AI(지면 충돌 방지 포함) 및 JSON 데이터 로딩 기능을 갖추고 있습니다.
...
- PyTorch (`pip install torch`) - RL 정책 예시에만 필요
- JSON 데이터 파일 (`f16_data_v2.json`, `uav_data_v2.json`)이 동일 디렉토리에 있어야 함
"""
import numpy as np
...
import torch            # RL 정책 예시용
import torch.nn as nn   # RL 정책 예시용
import matplotlib.pyplot as plt
import json
import os
import math
from copy import deepcopy
from unittest.mock import MagicMock # PID setpoint 설정 시 임시 사용
from datetime import datetime # 메인 실행 블록에서 시간 출력용
...
# --- 표준 대기 모델 (International Standard Atmosphere - ISA) ---
...
class AeroCoefficientInterpolator:
    ...
    def __init__(self, aero_data):
        ...
        # print(f"[Info] Aero interpolator initialized. Mach:{self.mach_range}, Alpha:[{np.rad2deg(self.alpha_range[0]):.1f}, {np.rad2deg(self.alpha_range[1]):.1f}] deg") # 로그 축소
...
class UAV_AeroCoefficientInterpolator(AeroCoefficientInterpolator):
     def __init__(self, aero_data):
         super().__init__(aero_data=aero_data); # print(f"[Info] UAV Aero interpolator initialized.") # 로그 축소
...
def find_trim_condition_optimized(model, target_speed, target_altitude, target_gamma_deg=0.0):
    ...
    # print(f"[Info] Finding trim for Speed={target_speed} m/s, Alt={target_altitude} m, Gamma={target_gamma_deg} deg...") # 로그 축소
    ...
    # print(f"  > Initial Guess [alpha, delta_e, throttle]: [{np.rad2deg(initial_guess[0]):.2f} deg, {np.rad2deg(initial_guess[1]):.2f} deg, {initial_guess[2]:.3f}]") # 로그 축소
...
# --------------------------------------------------------------------------
# Model 2: F16_6DOF_Model (Full Definition)
# --------------------------------------------------------------------------
class F16_6DOF_Model:
    """ F-16 6DOF 모델 (JSON 로딩, step 메소드 포함) """
    def __init__(self, initial_state, json_config_path="/mnt/hdd2/attoman/workspace/git/combat_sim/aero_db/f16_data_v2.json"):
        if not os.path.exists(json_config_path): raise FileNotFoundError(f"Config file not found: {json_config_path}")
        with open(json_config_path, 'r') as f: config = json.load(f); # print(f"[Info] Loaded config from: {json_config_path}")
        ...
        self.aero_data = config['aerodynamics']; self.engine_data = config['engine']; self.aircraft_type = config.get('aircraft_type', 'Unknown F-16')
        self.initial_state = np.array(initial_state, dtype=float); self.state = self.initial_state.copy()
        self.aero_interpolator = AeroCoefficientInterpolator(self.aero_data); print(f"[Info] {self.aircraft_type} 6DOF Model Initialized.")
    def _get_aero_derivatives(self, alpha_rad, mach): return self.aero_data['derivatives']
    def _get_engine_force_moment(self, state, controls):
        x, y, z, u, v, w, *_ = state; throttle = np.clip(controls.get('throttle', 0.0), 0.0, 1.0); altitude = -z; V = np.linalg.norm([u, v, w]); temp, press, rho, sound_speed = get_isa_conditions(altitude); mach = V / sound_speed if V > 1e-3 else 0.0
        max_thrust_sl = self.engine_data['max_thrust_sl_N']; alt_density_exp = self.engine_data.get('alt_density_exp', 1.0); mach_poly_sub = self.engine_data.get('mach_factor_subsonic_poly', [1.0]); mach_poly_super = self.engine_data.get('mach_factor_supersonic_poly', [1.0]); thrust_offset = np.array(self.engine_data.get('thrust_offset_m', [0,0,0]), dtype=float)
        density_factor = (rho / RHO0)**alt_density_exp; mach_factor = np.polyval(mach_poly_sub[::-1], mach) if mach < 1.0 else np.polyval(mach_poly_super[::-1], mach); thrust = throttle * max_thrust_sl * density_factor * mach_factor
        thrust_force = np.array([thrust, 0, 0]); thrust_moment = np.cross(thrust_offset, thrust_force); return thrust_force, thrust_moment
    def _get_aero_forces_moments(self, state, controls):
        ...
        delta_e=controls.get('elevator',0.0); delta_a=controls.get('aileron',0.0); delta_r=controls.get('rudder',0.0); alpha_dot = 0.0
        ...
        CL_base, CD_base, Cm_base = self.aero_interpolator.get_coeffs(mach, alpha); derivs = self._get_aero_derivatives(alpha, mach)
        CL=CL_base + derivs['CLq']*q_bar_dim + derivs['CLde']*delta_e + derivs.get('CLalpha_dot',0.0)*alpha_dot_bar; CD=CD_base + derivs.get('CDde',0.0)*delta_e
        Cm=Cm_base + derivs['Cmq']*q_bar_dim + derivs['Cmde']*delta_e + derivs.get('Cmalpha_dot',0.0)*alpha_dot_bar; CY=derivs['CYb']*beta + derivs['CYp']*p_bar + derivs['CYr']*r_bar + derivs.get('CYda',0.0)*delta_a + derivs['CYdr']*delta_r
        Cl=derivs['Clb']*beta + derivs['Clp']*p_bar + derivs['Clr']*r_bar + derivs['Clda']*delta_a + derivs.get('Cldr',0.0)*delta_r; Cn=derivs['Cnb']*beta + derivs['Cnp']*p_bar + derivs['Cnr']*r_bar + derivs.get('Cnda',0.0)*delta_a + derivs['Cndr']*delta_r
    ...
    def dynamics(self, t, state, controls_func):
        ...
        if abs(abs(theta) - np.pi/2) < 1e-4: d_phi_theta_psi = np.zeros(3) # Gimbal lock simple handle
        else: sec_theta=1/cth; tan_theta=sth*sec_theta; T_euler=np.array([[1, sph*tan_theta, cph*tan_theta],[0, cph, -sph],[0, sph*sec_theta, cph*sec_theta]]); d_phi_theta_psi = T_euler @ rates_body
    ...
    def simulate(self, t_span, t_eval, controls_func):
        ...
        sol = scipy.integrate.solve_ivp(fun=self.dynamics, t_span=t_span, y0=self.state, t_eval=t_eval, args=(controls_func,), method='RK45', rtol=1e-5, atol=1e-8)
        # print("[Info] Simulation finished.") # 로그 축소
        if sol.success: self.state = sol.y[:, -1]; return sol
...
# --------------------------------------------------------------------------
# Model 3: UAV_6DOF_Controlled_Model (Full Definition)
# --------------------------------------------------------------------------
class UAV_6DOF_Controlled_Model(F16_6DOF_Model): # Inherit structure, override methods
    """ UAV 6DOF 모델 + 제어기 (JSON 로딩, 개선된 LQR) """
    def __init__(self, initial_state, json_config_path="/mnt/hdd2/attoman/workspace/git/combat_sim/aero_db/uav_data_v2.json", control_config=None):
        if not os.path.exists(json_config_path): raise FileNotFoundError(f"Config file not found: {json_config_path}")
        with open(json_config_path, 'r') as f: config = json.load(f); # print(f"[Info] Loaded config from: {json_config_path}")
        params = config['parameters']; self.mass=float(params['mass_kg']); self.inertia_tensor=np.array(params['inertia_tensor_kgm2'],dtype=float); self.inv_inertia_tensor=np.linalg.inv(self.inertia_tensor)
        self.ref_area=float(params['ref_area_m2']); self.ref_chord=float(params['ref_chord_m']); self.ref_span=float(params['ref_span_m']); self.g=G
        self.aero_data=config['aerodynamics']; self.engine_data=config['engine']; self.aircraft_type=config.get('aircraft_type', 'Unknown UAV')
        self.initial_state = np.array(initial_state, dtype=float); self.state = self.initial_state.copy()
        self.aero_interpolator = UAV_AeroCoefficientInterpolator(self.aero_data) # Use UAV interpolator
        self.control_config = control_config if control_config else {'type': None}; self.controllers = {}; self.lqr_gain = None; self.lqr_trim_state = None; self.lqr_trim_controls = None
        self.lqr_state_indices = []; self.lqr_control_keys = []; self.lqr_target_state_selected = None
        self._setup_controllers(); print(f"[Info] {self.aircraft_type} 6DOF Controlled Model Initialized.")
    def _get_aero_derivatives(self, alpha_rad, mach): return self.aero_data['derivatives']
    def _get_engine_force_moment(self, state, controls):
    ...
    def _setup_controllers(self):
        ctype = self.control_config.get('type'); params = self.control_config.get('params', {}); dt = self.control_config.get('dt', 0.02)
        if ctype == 'PID':
            print("[Info] Initializing PID controllers."); lim_ele = (-np.deg2rad(20), np.deg2rad(20)); lim_ail = (-np.deg2rad(25), np.deg2rad(25)); lim_rud = (-np.deg2rad(30), np.deg2rad(30)); lim_thr = (0.0, 1.0)
            ...
            if 'airspeed' in params: self.controllers['airspeed'] = PIDController(params['airspeed']['Kp'], params['airspeed']['Ki'], params['airspeed']['Kd'], setpoint=params['airspeed']['setpoint'], dt=dt, output_limits=lim_thr)
            ...
            print("[Info] Setting up LQR controller..."); trim_params = params.get('trim_condition', {'speed': 50, 'altitude': 1000})
            ...
            self.lqr_trim_state = trim_state_found; self.lqr_trim_controls = trim_controls_found; self.lqr_state_indices = params.get('state_indices', [3, 5, 10, 7]); self.lqr_control_keys = params.get('control_keys', ['elevator', 'throttle'])
            ...
            Q_diag = params.get('Q_diag', [0.1,0.1,1,1]); R_diag = params.get('R_diag', [1,0.5]); Q = np.diag(Q_diag); R = np.diag(R_diag)
            ...
            try: P = scipy.linalg.solve_continuous_are(A, B, Q, R); self.lqr_gain = np.linalg.inv(R) @ B.T @ P; self.lqr_target_state_selected = self.lqr_trim_state[self.lqr_state_indices]; print(f"[Info] LQR gain K computed (shape {self.lqr_gain.shape}).")
            except Exception as e: print(f"[Error] LQR setup failed: {e}"); self.control_config['type'] = None
        else: print("[Warning] Unknown or no controller type specified for UAV.")
    def get_control_inputs(self, t, state):
        ctype = self.control_config.get('type'); controls = self.lqr_trim_controls.copy() if ctype == 'LQR' and self.lqr_trim_controls else {'throttle': 0.0, 'elevator': 0.0, 'aileron': 0.0, 'rudder': 0.0}
        if ctype == 'PID':
            ...
            target_pitch = self.controllers.get('altitude_target', lambda x:0)(alt) if 'altitude_target' in self.controllers else self.controllers.get('pitch', MagicMock(setpoint=0)).setpoint
            if 'pitch' in self.controllers: self.controllers['pitch'].set_setpoint(target_pitch)
            target_roll = self.controllers.get('heading_target', lambda x:0)(hdg) if 'heading_target' in self.controllers else self.controllers.get('roll', MagicMock(setpoint=0)).setpoint
            if 'roll' in self.controllers: self.controllers['roll'].set_setpoint(target_roll)
            ...
            current_lqr_state = state[self.lqr_state_indices]; state_error = current_lqr_state - self.lqr_target_state_selected; control_deviation = -self.lqr_gain @ state_error
            for i, key in enumerate(self.lqr_control_keys): controls[key] = self.lqr_trim_controls[key] + control_deviation[i]
            limits = {'elevator': (-np.deg2rad(20), np.deg2rad(20)), 'throttle': (0.0, 1.0), 'aileron': (-np.deg2rad(25), np.deg2rad(25)), 'rudder': (-np.deg2rad(30), np.deg2rad(30))}
            for key in controls:
    ...
    def simulate(self, t_span, t_eval):
        # Uses fixed-step loop for better control interaction
        ...
        for i in range(num_steps-1):
            t=time_points[i]; current_controls=self.get_control_inputs(t, current_state); control_inputs_history.append(current_controls.copy())
            self.step(dt, current_controls); current_state = self.state; states[:, i+1] = current_state
        print("[Info] UAV Controlled Sim finished."); from scipy.integrate import OdeResult; sol=OdeResult(t=time_points, y=states); sol.control_inputs=control_inputs_history; return sol
    def _normalize_angle(self, angle): return _normalize_angle(angle)

# --------------------------------------------------------------------------
# Model 4: CombatUAV_Agent & MultiAgent_Simulation (Full Definitions)
# --------------------------------------------------------------------------
class SensorModel:
    """ 간단한 센서 모델 """
    ...
    def detect(self, self_state, other_states):
        ...
        for agent_id, other_state in other_states.items():
            ...
            if distance > 1e-6 and distance <= self.max_range:
                vector_to_other_normalized = vector_to_other / distance; dot_prod = np.dot(forward_vector_ned, vector_to_other_normalized)
                angle_rad = np.arccos(np.clip(dot_prod, -1.0, 1.0))
                if abs(angle_rad) <= self.fov_rad / 2:
                    is_los_clear = True # TODO: LOS check
...
class CombatUAV_Agent(UAV_6DOF_Controlled_Model):
    """ 다중 에이전트용 UAV 클래스 """
    def __init__(self, agent_id, initial_state, model_json_config_path="uav_data_v2.json", control_config=None, sensor_config=None, policy_type='rule_based', rl_policy_net=None, team_id=None):
        super().__init__(initial_state=initial_state, json_config_path=model_json_config_path, control_config=control_config); self.id = agent_id; self.team_id = team_id; self.state = np.array(initial_state, dtype=float)
        self.alive = True; self.sensor_config = sensor_config if sensor_config else {'range': 5000, 'fov': 120}; self.policy_type = policy_type; self.rl_policy_net = rl_policy_net; self.detected_info_cache = []
        self.current_controls = {'throttle':0.0, 'elevator':0.0, 'aileron':0.0, 'rudder':0.0}
        if control_config and control_config.get('type') == 'LQR' and self.lqr_trim_controls: self.current_controls = self.lqr_trim_controls.copy()
        self.sensor = SensorModel(max_range=self.sensor_config['range'], fov_deg=self.sensor_config['fov']); # print(f"[Info] Combat UAV Agent {self.id} initialized.") # 로그 축소
    def perceive(self, all_agent_states):
        if not self.alive: return []; other_states = {aid: agent['state'] for aid, agent in all_agent_states.items() if aid != self.id and agent['alive']}; self.detected_info_cache = self.sensor.detect(self.state, other_states); return self.detected_info_cache
    def decide(self, mission_goal=None, all_agent_states=None):
        if not self.alive: return {'throttle': 0.0, 'elevator': 0.0, 'aileron': 0.0, 'rudder': 0.0}; detected_info = self.detected_info_cache; controls = self.current_controls.copy()
        if self.policy_type == 'rule_based':
            enemies = [d for d in detected_info if all_agent_states[d['id']].get('team_id') != self.team_id] if all_agent_states else []; closest_enemy = min(enemies, key=lambda x: x['distance']) if enemies else None
            if closest_enemy: enemy_pos=closest_enemy['state_observed'][0:3]; self_pos=self.state[0:3]; vector_to_enemy = enemy_pos - self_pos; target_heading = math.atan2(vector_to_enemy[1], vector_to_enemy[0]); heading_error = _normalize_angle(target_heading - self.state[8]); target_roll = np.clip(heading_error * 1.5, -np.deg2rad(45), np.deg2rad(45));
            if 'roll' in self.controllers: self.controllers['roll'].set_setpoint(target_roll);
            if 'altitude' in self.controllers: self.controllers['altitude'].set_setpoint(-closest_enemy['state_observed'][2]); controls = self.get_control_inputs(0, self.state)
            else:
                 if mission_goal and 'waypoint' in mission_goal: wp = mission_goal['waypoint']; vector_to_wp = wp - self.state[0:3]; target_heading = math.atan2(vector_to_wp[1], vector_to_wp[0]); heading_error = _normalize_angle(target_heading - self.state[8]); target_roll = np.clip(heading_error * 1.0, -np.deg2rad(30), np.deg2rad(30));
                 if 'roll' in self.controllers: self.controllers['roll'].set_setpoint(target_roll);
                 if 'altitude' in self.controllers and 'altitude' in mission_goal: self.controllers['altitude'].set_setpoint(mission_goal['altitude']);
                 if 'airspeed' in self.controllers and 'speed' in mission_goal: self.controllers['airspeed'].set_setpoint(mission_goal['speed']); controls = self.get_control_inputs(0, self.state)
                 else:
                    if 'roll' in self.controllers: self.controllers['roll'].set_setpoint(0.0);
                    if 'pitch' in self.controllers: self.controllers['pitch'].set_setpoint(0.0); controls = self.get_control_inputs(0, self.state)
        elif self.policy_type == 'rl':
             if self.rl_policy_net is None: raise ValueError("RL policy net missing."); rl_input_state = self._prepare_rl_input(self.state, detected_info, mission_goal, all_agent_states); rl_input_tensor = torch.FloatTensor(rl_input_state).unsqueeze(0); self.rl_policy_net.eval();
             with torch.no_grad(): action_output = self.rl_policy_net(rl_input_tensor); controls = self._interpret_rl_action(action_output)
        self.current_controls = controls; return controls
    def _prepare_rl_input(self, self_state, detected_info, mission_goal, all_agent_states):
         input_vec = np.zeros(24); input_vec[0:3]=self_state[3:6]/100.0; input_vec[3:6]=self_state[9:12]/np.pi; input_vec[6:8]=self_state[6:8]/(np.pi/2); input_vec[8]=-self_state[2]/5000.0
         def get_relative_info(target_list, max_dist):
            info = np.zeros(6); closest = min(target_list, key=lambda x: x['distance']) if target_list else None
            if closest: rel_pos=closest['state_observed'][0:3]-self_state[0:3]; other_vel=closest['state_observed'][3:6] if len(closest['state_observed'])>6 else np.zeros(3); rel_vel=other_vel-self_state[3:6]; dist=closest['distance']; info[0]=dist/max_dist; info[1]=math.atan2(rel_pos[1],rel_pos[0])/np.pi; info[2]=math.asin(np.clip(rel_pos[2]/max(dist,1e-6),-1,1))/(np.pi/2); info[3:6]=rel_vel/100.0
            return info
         enemies = [d for d in detected_info if all_agent_states[d['id']].get('team_id') != self.team_id] if all_agent_states else []; allies = [d for d in detected_info if all_agent_states[d['id']].get('team_id') == self.team_id] if all_agent_states else []
         input_vec[9:15]=get_relative_info(enemies, self.sensor_config['range']); input_vec[15:21]=get_relative_info(allies, self.sensor_config['range'])
         if mission_goal and 'waypoint' in mission_goal: wp_vec = mission_goal['waypoint']-self_state[0:3]; wp_dist = np.linalg.norm(wp_vec); input_vec[21:24] = wp_vec / max(wp_dist, 1.0)
         return input_vec
    def _interpret_rl_action(self, action_output):
         action_values = action_output.squeeze(0).cpu().numpy(); controls = {'throttle': (action_values[0]+1.0)/2.0, 'elevator': action_values[1]*np.deg2rad(20), 'aileron': action_values[2]*np.deg2rad(25), 'rudder': action_values[3]*np.deg2rad(30)}; return controls
    # step method is inherited
    def check_collision_or_damage(self, all_agent_states):
         if not self.alive: return; collision_radius = 5.0; self_pos = self.state[0:3]
         for other_id, other_agent_info in all_agent_states.items():
            if other_id != self.id and other_agent_info['alive']: other_pos = other_agent_info['state'][0:3];
            if np.linalg.norm(self_pos - other_pos) < collision_radius: self.alive = False; print(f"[Event] Agent {self.id} collided with Agent {other_id}."); return
class MultiAgent_Simulation:
    def __init__(self, agents, mission_goals=None): self.agents={agent.id: agent for agent in agents}; self.agent_states={aid:{'state':agent.state.copy(),'alive':agent.alive,'team_id':agent.team_id} for aid,agent in self.agents.items()}; self.mission_goals=mission_goals if mission_goals else {}; self.time=0.0; self.history=[]
    def run(self, duration, dt):
        num_steps=int(duration/dt); print(f"[Info] Starting Multi-Agent Sim for {duration}s dt={dt}s"); self._record_state()
        for i in range(num_steps):
            self.time=(i+1)*dt # Use time at end of step interval for logging consistency
            if i%int(max(1,5/dt))==0: alive=[aid for aid,info in self.agent_states.items() if info['alive']]; print(f"\n--- Sim Time: {self.time:.2f}s | Alive: {len(alive)} ({alive}) ---")
            current_states_snapshot = {aid: info for aid, info in self.agent_states.items()}; agent_controls={}
            for agent_id, agent in self.agents.items(): # Perceive & Decide
                if self.agent_states[agent_id]['alive']: agent.perceive(current_states_snapshot); mission=self.mission_goals.get(agent_id); agent_controls[agent_id]=agent.decide(mission, current_states_snapshot)
            for agent_id, agent in self.agents.items(): # Step
                if self.agent_states[agent_id]['alive']: controls=agent_controls.get(agent_id,{}); agent.step(dt, controls); self.agent_states[agent_id]['state']=agent.state.copy()
            current_states_after_step = {aid: info for aid, info in self.agent_states.items()} # Check Collision
            destroyed=[]
            for agent_id, agent in self.agents.items():
                if self.agent_states[agent_id]['alive']: was_alive=agent.alive; agent.check_collision_or_damage(current_states_after_step);
                if was_alive and not agent.alive: self.agent_states[agent_id]['alive']=False; destroyed.append(agent_id)
            self._record_state() # Record state at end of interval
    ...
    def _record_state(self): state_snapshot={'time':self.time,'agents':deepcopy(self.agent_states)}; self.history.append(state_snapshot)
...
def calculate_bfm_parameters(self_state, opponent_state):
    ...
    rel_vel_ned = opp_vel_n - self_vel_n; params['closure_rate_mps'] = -np.dot(rel_vel_ned, los_vec_ned_unit)
    params['delta_alt_m'] = -self_state[2] - (-opponent_state[2])
    # AOT: Angle between Opponent's Tail Vector (NED) and LOS Vector (NED, Self to Opponent)
    ...
    # Aspect: Angle between Self's Tail Vector (NED) and LOS Vector (NED, Opponent to Self)
...
def execute_offensive_controls(self_state, opponent_state, bfm_params):
    # TODO: Implement proper Lag Pursuit logic
    current_alt = -self_state[2]; max_g = 8.0 if current_alt > 1500 else max(1.5, 8.0 * (current_alt / 1500)**0.5)
    controls = get_simple_bfm_controls(self_state, opponent_state, target_speed_kts=480, max_g=max_g); return controls
...
# --- 상태 머신 AI 함수 ---
def get_state_machine_bfm_controls(aircraft_id, current_state, opponent_state, current_bfm_state):
    current_alt = -current_state[2]; next_bfm_state = current_bfm_state; state_changed = False # 상태 변경 여부 플래그
    if current_alt < ALTITUDE_CRITICAL:
        ...
        bfm_params = calculate_bfm_parameters(current_state, opponent_state)
        threat_range=4000; threat_aspect=70; adv_range=6000; adv_aot=60
        threat_escape_range=threat_range+2000; threat_escape_aspect=threat_aspect+50
        adv_lose_range=adv_range+1000; adv_lose_aot=adv_aot+30
        prev_state = current_bfm_state # 상태 변경 전 상태 저장
        if current_bfm_state == STATE_NEUTRAL:
            if i_am_offensive(bfm_params, adv_aot, adv_range): next_bfm_state = STATE_OFFENSIVE
            elif opponent_is_threat(bfm_params, threat_aspect, threat_range): next_bfm_state = STATE_DEFENSIVE
            ...
            if not opponent_is_threat(bfm_params, threat_escape_aspect, threat_escape_range): next_bfm_state = STATE_NEUTRAL
            elif i_am_offensive(bfm_params, adv_aot, adv_range): next_bfm_state = STATE_OFFENSIVE
        if prev_state != next_bfm_state: state_changed = True # 상태 변경 감지

    state_names = {0:"NEUTRAL", 1:"OFFENSIVE", 2:"DEFENSIVE", 3:"RECOVERY"}
    if state_changed: print(f"  > {aircraft_id} State: {state_names.get(current_bfm_state, '?')} -> {state_names.get(next_bfm_state, '?')}")

    if next_bfm_state == STATE_RECOVERY: controls = execute_recovery_controls(current_state)
    elif next_bfm_state == STATE_OFFENSIVE: controls = execute_offensive_controls(current_state, opponent_state, bfm_params)
    elif next_bfm_state == STATE_DEFENSIVE: controls = execute_defensive_controls(current_state, opponent_state, bfm_params)
    else: controls = execute_neutral_controls(current_state, opponent_state, bfm_params)
    return controls, next_bfm_state

# --- 1v1 시뮬레이션 메인 루프 (상태 머신 AI 사용) ---
def run_1v1_simulation_sm(aircraft1, aircraft2, initial_state1, initial_state2, duration, dt):
    aircraft1.state = initial_state1.copy(); aircraft1.bfm_state = STATE_NEUTRAL
    ...
    print(f"[Info] Starting 1v1 SM Simulation: {aircraft1.aircraft_type} vs {aircraft2.aircraft_type} ({duration}s, dt={dt}s)")
    current_time = 0.0
    for i in range(num_steps):
        t = time_points[i]; state1 = aircraft1.state; state2 = aircraft2.state
        controls1, next_state1_bfm = get_state_machine_bfm_controls("AC1", state1, state2, aircraft1.bfm_state)
        controls2, next_state2_bfm = get_state_machine_bfm_controls("AC2", state2, state1, aircraft2.bfm_state)
        aircraft1.bfm_state = next_state1_bfm; aircraft2.bfm_state = next_state2_bfm
        current_state1 = aircraft1.step(dt, controls1); current_state2 = aircraft2.step(dt, controls2)
        current_time = t # Log time at beginning of step
        ...
        if distance > 30000: print(f"[Event] Separated @ t={current_time:.2f}s. Dist: {distance:.1f}m"); break
        ...
        # Reduce print frequency further
        if i % int(max(1, 10/dt)) == 0: print(f"  t={current_time:.1f}s, Dist={distance:.0f}m, Alt1={alt1_msl:.0f}m, Alt2={alt2_msl:.0f}m, St1={aircraft1.bfm_state}, St2={aircraft2.bfm_state}")
    if history: print(f"[Info] 1v1 SM Sim finished at t={history[-1]['time']:.2f}s")
    else: print("[Info] 1v1 SM Sim did not run.")
    return history

# ==========================================================================
# Main Execution Block (State Machine 1v1 Example - F16 vs UAV)
# ==========================================================================
if __name__ == "__main__":
    print(f"Current Time: {datetime.now()}")
    f16_config_file = "/mnt/hdd2/attoman/workspace/git/combat_sim/aero_db/f16_data_v2.json"; uav_config_file = "/mnt/hdd2/attoman/workspace/git/combat_sim/aero_db/uav_data_v2.json"
    if not os.path.exists(f16_config_file): print(f"[ERROR] F-16 config not found: {f16_config_file}"); exit()
    if not os.path.exists(uav_config_file): print(f"[WARNING] UAV config file not found: {uav_config_file}")

    # 초기 조건: F-16이 UAV 약간 뒤에서 시작
    alt_m = 5000 # m
    spd_f16_mps = 400 * 0.5144
    spd_uav_mps = 150 * 0.5144
    sep_m = 3000 # m (F-16이 UAV 뒤 3km)

    # UAV(AC2): 북쪽으로 비행 시작
    initial_state_2 = np.array([0, 0, -alt_m, spd_uav_mps, 0, 0, 0, 0, 0.0, 0, 0, 0])
    # F-16(AC1): UAV 뒤에서 약간 높은 고도, 더 빠른 속도로 북쪽 비행 시작
    initial_state_1 = np.array([-sep_m, 0, -(alt_m + 500), spd_f16_mps, 0, 0, 0, 0, 0.0, 0, 0, 0])

    aircraft_1 = F16_6DOF_Model(initial_state=initial_state_1, json_config_path=f16_config_file)
    aircraft_2 = UAV_6DOF_Controlled_Model(initial_state=initial_state_2, json_config_path=uav_config_file) # Use UAV model

    sim_duration = 120; time_step = 0.05
    simulation_history_sm = run_1v1_simulation_sm(aircraft_1, aircraft_2, initial_state_1, initial_state_2, sim_duration, time_step)

    # Visualization
    if simulation_history_sm:
        state_names_plot = {0: "Neutral", 1: "Offensive", 2: "Defensive", 3: "Recovery"}
        times=[entry['time'] for entry in simulation_history_sm]
        pos1_x=[e['aircraft1']['state'][0] for e in simulation_history_sm]; pos1_y=[e['aircraft1']['state'][1] for e in simulation_history_sm]
        pos2_x=[e['aircraft2']['state'][0] for e in simulation_history_sm]; pos2_y=[e['aircraft2']['state'][1] for e in simulation_history_sm]
        alt1=[-e['aircraft1']['state'][2] for e in simulation_history_sm]; alt2=[-e['aircraft2']['state'][2] for e in simulation_history_sm]
        state1_hist=[e['aircraft1']['bfm_state'] for e in simulation_history_sm]
        state2_hist=[e['aircraft2']['bfm_state'] for e in simulation_history_sm]
        fig, axs = plt.subplots(3, 1, figsize=(10, 14))
        axs[0].plot(pos1_y, pos1_x, 'b-', label=f'{aircraft_1.aircraft_type} 1'); axs[0].plot(pos2_y, pos2_x, 'r-', label=f'{aircraft_2.aircraft_type} 2'); axs[0].plot(pos1_y[0], pos1_x[0], 'bo', ms=8, label='Start 1'); axs[0].plot(pos2_y[0], pos2_x[0], 'ro', ms=8, label='Start 2'); axs[0].set_xlabel("East (m)"); axs[0].set_ylabel("North (m)"); axs[0].set_title("1v1 BFM Simulation (SM AI + Recovery)"); axs[0].legend(); axs[0].grid(True); axs[0].axis('equal')
        axs[1].plot(times, alt1, 'b-', label='Alt 1'); axs[1].plot(times, alt2, 'r-', label='Alt 2'); axs[1].axhline(ALTITUDE_CRITICAL, color='k', linestyle=':', label=f'Crit Alt'); axs[1].axhline(ALTITUDE_RECOVERED, color='k', linestyle='--', label=f'Rec Alt'); axs[1].set_xlabel("Time (s)"); axs[1].set_ylabel("Altitude (m MSL)"); axs[1].legend(); axs[1].grid(True); axs[1].set_ylim(bottom=0)
        axs[2].plot(times, state1_hist, 'b.', markersize=2, label='State 1'); axs[2].plot(times, state2_hist, 'r.', markersize=2, label='State 2'); axs[2].set_xlabel("Time (s)"); axs[2].set_ylabel("BFM State"); axs[2].legend(); axs[2].grid(True); axs[2].set_yticks(list(state_names_plot.keys())); axs[2].set_yticklabels(list(state_names_plot.values()))
        plt.tight_layout(); plt.savefig("./1v1_bfm_sm_log_f16_vs_uav.png"); plt.show() # Save figure

        # Optional: Save log data
        try:
            log_filename = "./1v1_bfm_sm_log_f16_vs_uav.json"
            serializable_history = []
            for entry in simulation_history_sm:
                # Convert numpy arrays to lists for JSON serialization
                entry['aircraft1']['state'] = entry['aircraft1']['state'].tolist()
                entry['aircraft2']['state'] = entry['aircraft2']['state'].tolist()
                # Controls might contain numpy types, ensure they are basic types
                entry['aircraft1']['controls'] = {k: float(v) for k, v in entry['aircraft1']['controls'].items()}
                entry['aircraft2']['controls'] = {k: float(v) for k, v in entry['aircraft2']['controls'].items()}
                serializable_history.append(entry)
            with open(log_filename, "w") as f: json.dump(serializable_history, f, indent=2)
            print(f"[Info] Simulation history saved to {log_filename}")
        except Exception as e: print(f"[Error] Failed to save simulation log: {e}")
